   ...
   <>
    ...
    <form className='mt-8' onSubmit={(e) => {e.preventDefault(); formik.handleSubmit(e)} } >
      <div className="inputDiv" >
        <input value={formik.values.name} onChange={formik.handleChange} id='name' name='name'  type="text" className={`peer input placeholder-transparent ${formik.errors.name ? 'border-red-500' : ''} `}   placeholder='name' />
        <label  className='absolute bottom-2.5 text-gray-500 text-base transition-all duration-300 ease-in' >Name</label>
        <div className="line"></div>
        { formik.errors.name && <ErrorMessage message={formik.errors.name} /> }
      </div>

      <div className="inputDiv">
        <input placeholder='email' name='email' onChange={formik.handleChange} value={formik.values.email} className={`input peer ${formik.errors.name ? 'border-red-500' : ''}`} type="text"/>
        <div className="line"></div>
        <label className='absolute bottom-2.5 text-gray-500 text-base transition-all duration-300 ease-in'>Email Address</label>
